        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {allTags.slice(0, 15).map((tag) => (
            <Link
              ...
              className={`text-xs px-3 py-1 rounded-full border transition-colors ${techColors[tag] || "bg-white/5 border-white/10 text-gray-400 hover:text-white"}`}
            >
              {tag}
            </Link>
          ))}
      ...
      {projects.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project) => (
            <div
              key={project.id}
              className="group bg-white/3 border border-white/8 rounded-2xl overflow-hidden hover:bg-white/5 hover:border-white/15 transition-all"
            >
              {/* Card header */}
              <div className="p-6 pb-4">
                ...
                <h3 className="font-semibold text-white text-lg mb-2 group-hover:text-[#FECB00] transition-colors">
                  {project.title}
                </h3>
              ...
              <div className="px-6 pb-4">
                <div className="flex flex-wrap gap-1.5">
                  {project.tags.split(",").map((tag) => (
                    <span
                      key={tag}
                      className={`text-xs px-2 py-0.5 rounded-full ${techColors[tag.trim()] || "bg-white/5 text-gray-500"}`}
                    >
                      {tag.trim()}
                    </span>
                  ))}
              ...
              <div className="px-6 py-4 border-t border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-600">
                    {format(new Date(project.createdAt), "MMM yyyy", { locale: fr })}
                  </span>
                </div>
