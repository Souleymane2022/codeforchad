  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        ...
        <Link href="/community/new" className="flex items-center gap-2 px-5 py-2.5 bg-[#FECB00] text-[#0a0f1e] font-semibold text-sm rounded-xl hover:bg-[#FECB00]/90 transition-colors">
          <Plus size={16} />
          Nouvelle discussion
        </Link>
      </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white/3 border border-white/8 rounded-2xl p-4 sticky top-24">
            ...
            <div className="space-y-1">
              <Link href="/community" className={`flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors ${!category ? "bg-[#FECB00]/10 text-[#FECB00]" : "text-gray-400 hover:text-white hover:bg-white/5"}`}>
                <span>Toutes</span>
                <span className="text-xs text-gray-600">{posts.length}</span>
              </Link>
              {categories.map((cat) => (
                <Link key={cat.id} href={`/community?category=${cat.slug}`} className={`flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors ${category === cat.slug ? "bg-[#FECB00]/10 text-[#FECB00]" : "text-gray-400 hover:text-white hover:bg-white/5"}`}>
        ...
        <div className="lg:col-span-3">
          <div className="flex items-center gap-2 text-xs text-gray-500 mb-4">
            ...
            {category && (
              <>
                <span>·</span>
                <span className="text-[#FECB00]">{categories.find((c) => c.slug === category)?.name}</span>
              </>
            )}
          </div>
              ...
              posts.map((post) => (
                <Link key={post.id} href={`/community/${post.id}`} className="flex gap-4 bg-white/3 border border-white/8 rounded-xl p-4 hover:bg-white/6 hover:border-white/15 transition-all group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="font-medium text-white group-hover:text-[#FECB00] transition-colors">{post.title}</h3>
                    <div className="flex items-center gap-4 mt-1.5 text-xs text-gray-500">
                      ...
                      <span className="flex items-center gap-1"><MessageSquare size={11} /> {post._count.replies}</span>
